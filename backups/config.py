...
def toggle_clock_format():
    ...
    def _toggle(qtile):
        ...
        qtile.widgets_map['clock_toggle'].update()
    return _toggle
    ...
    Screen(
        top=bar.Bar(
            [
                ...
                widget.GenPollText(
                    name='clock_toggle',
                    func=lambda: f"<span color='#9c27b0' size='large'>󰥔</span> {subprocess.run(['date', '+%-l:%M %p'], capture_output=True, text=True).stdout.strip()}" if clock_showing_time[0] else f"<span color='#80bfff' size='large'>󰸗</span> {subprocess.run(['date', '+%a %b %-d'], capture_output=True, text=True).stdout.strip()}",
                    update_interval=1,
                    markup=True,
                    padding=6,
                    mouse_callbacks={'Button1': lazy.function(toggle_clock_format())}
                ),
                widget.Spacer(),
